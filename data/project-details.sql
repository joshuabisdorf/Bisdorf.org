...
INSERT INTO project_page_copy (project_id, overview, what_built) VALUES ('secure-ai-gateway', 'Secure AI Gateway is a security-focused control plane and OpenAI-compatible proxy for LLM applications. I built it to keep security policy outside the model and to make authentication, authorization, rate limiting, usage controls, PII handling, prompt-injection controls, tool authorization, auditing, and observability enforceable at a trusted boundary.', 'I designed and implemented the gateway as a production-style security boundary around model traffic. The system combines gateway-issued API keys, PostgreSQL-backed identity and usage state, Redis-backed distributed rate limiting and one-time execution claims, model and tool authorization, PII and prompt-injection policy, metadata-only telemetry, container and Kubernetes hardening, and release verification. I also built deterministic adversarial, resilience, concurrency, performance, and supply-chain checks so the security claims are continuously testable.');
INSERT INTO project_highlights (project_id, body, position) VALUES ('secure-ai-gateway', 'Built an OpenAI-compatible FastAPI gateway that separates authentication from model, usage, PII, prompt-injection, tool, and execution authorization decisions.', 0);
INSERT INTO project_highlights (project_id, body, position) VALUES ('secure-ai-gateway', 'Made PostgreSQL authoritative for client identity and persistent usage while Redis provides distributed rate-limit state and one-time execution-ticket replay protection.', 1);
INSERT INTO project_highlights (project_id, body, position) VALUES ('secure-ai-gateway', 'Added structured and semantic PII controls, prompt-injection audit and deny modes, least-privilege tool exposure, and execution-time re-authorization.', 2);
INSERT INTO project_highlights (project_id, body, position) VALUES ('secure-ai-gateway', 'Hardened the deployment with non-root containers, restricted Kubernetes Pod Security, default-deny NetworkPolicies, resource limits, probes, and disruption controls.', 3);
INSERT INTO project_highlights (project_id, body, position) VALUES ('secure-ai-gateway', 'Published v1.0.0 with reproducible dependency locking, CodeQL and container scanning, SBOM and provenance attestations, immutable GHCR images, and whole-repository release gates.', 4);
INSERT INTO project_extra_details (project_id, label, value, position) VALUES ('secure-ai-gateway', 'Release', 'v1.0.0', 0);
INSERT INTO project_extra_details (project_id, label, value, position) VALUES ('secure-ai-gateway', 'Interface', 'OpenAI-compatible chat completions gateway', 1);
INSERT INTO project_extra_sections (project_id, title, content, position) VALUES ('secure-ai-gateway', 'Security Model', 'Model input, model output, and model-proposed tool calls are treated as untrusted. Security policy is enforced outside the model, and security-critical shared state remains authoritative across replicas. Tool execution requires a short-lived signed ticket plus an independent execution-time authorization check.', 0);
INSERT INTO project_extra_sections (project_id, title, content, position) VALUES ('secure-ai-gateway', 'Verification', 'The repository includes deterministic security regression tests, prompt-injection and semantic-PII benchmarks, dependency-failure injection, two-replica concurrency and failover checks, bounded performance tests, live Kubernetes policy validation, Terraform validation, CodeQL, container scanning, repository preflight, and release artifact verification.', 1);
